...
export default function TrustBar({ dict, locale }: { dict: Dict; locale: Locale }) {
  return (
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="border-l border-stone pl-4">
            ...
            <p className="mt-1 text-xs text-bark-600">
              {reviewSummary.label[locale]} · {reviewSummary.count} {dict.trust.ratingLabel}
            </p>
          </div>
        </div>
      ...
      <div className="shell border-t border-stone/60 py-4">
        <ul className="flex flex-wrap items-center justify-center gap-x-8 gap-y-2.5 md:justify-between">
          {dict.trust.items.map((item) => (
            <li key={item} className="flex items-center gap-2 text-[0.82rem] text-bark-700">
              <Check />
              {item}
            </li>
          ))}
...
const Star = () => (
  <svg width="14" height="14" viewBox="0 0 20 20" fill="currentColor" className="text-clay-500" aria-hidden="true">
    <path d="M10 1.5l2.6 5.3 5.9.85-4.25 4.15 1 5.85L10 14.9l-5.25 2.75 1-5.85L1.5 7.65l5.9-.85L10 1.5z" />
  </svg>
);

const Check = () => (
  <svg width="15" height="15" viewBox="0 0 16 16" fill="none" className="shrink-0 text-clay-600" aria-hidden="true">
