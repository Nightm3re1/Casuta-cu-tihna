...
import { ctaShot } from '@/content/gallery';
...
export default function FinalCTA({ dict, locale }: { dict: Dict; locale: Locale }) {
  ...
  return (
    <section id="book" className="relative overflow-hidden bg-bark-950 py-24 text-cream md:py-32" aria-labelledby="book-title">
      {/* The terrace at dusk, held well back so the form stays legible. */}
      <div className="absolute inset-0 opacity-[0.22]">
        <Photo slug={shot.id} alt="" sizes="100vw" />
      </div>
      <div className="absolute inset-0 bg-[radial-gradient(90%_70%_at_50%_45%,rgba(10,18,14,0.72)_0%,#0A120E_78%)]" />

      <div className="shell relative">
        <Reveal className="mx-auto max-w-2xl text-center">
          <p className="eyebrow !text-clay-300">{dict.finalCta.eyebrow}</p>
        ...
        <Reveal delay={120} className="mx-auto mt-11 max-w-2xl">
          ...
          <ul className="mt-7 flex flex-wrap items-center justify-center gap-x-7 gap-y-2.5">
            {dict.finalCta.reassure.map((item) => (
              <li key={item} className="flex items-center gap-2 text-[0.82rem] text-cream/60">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="none" className="shrink-0 text-clay-300" aria-hidden="true">
          ...
          <p className="mt-8 text-center text-sm text-cream/65">
            <a href={site.contact.phoneHref} className="underline-offset-4 transition-colors hover:text-clay-200 hover:underline">
              {site.contact.phone}
            </a>
            <span className="mx-3 opacity-40">·</span>
            <a href={`mailto:${site.contact.email}`} className="underline-offset-4 transition-colors hover:text-clay-200 hover:underline">
              {site.contact.email}
            </a>
          </p>
        </Reveal>
