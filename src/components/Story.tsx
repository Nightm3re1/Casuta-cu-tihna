...
import { storyShot } from '@/content/gallery';
    ...
    <section id="story" className="relative overflow-hidden py-24 md:py-36" aria-labelledby="story-title">
      <div className="shell grid items-center gap-14 lg:grid-cols-[1fr_0.9fr] lg:gap-20">
        <Reveal>
          ...
          <h2 id="story-title" className="mt-4 max-w-xl text-display-lg font-semibold text-bark-900">
            {dict.story.title}
          </h2>
          <div className="mt-7 space-y-5 text-[1.02rem] leading-relaxed text-bark-700">
            {dict.story.body.map((para) => (
              <p key={para.slice(0, 24)}>{para}</p>
            ))}
          </div>

          <figure className="mt-10 border-l-2 border-clay-500 pl-6">
        ...
        <Reveal delay={120}>
          <figure className="relative">
            <div className="relative aspect-[4/5] overflow-hidden arch bg-linen">
              <Photo slug={shot.id} alt={shot.alt[locale]} sizes="(max-width: 1024px) 92vw, 42vw" />
            </div>
            {/* Year plate, struck like the one on the beam. */}
            <div className="absolute -bottom-6 -left-3 rounded-sm bg-bark-900 px-6 py-4 text-cream shadow-xl md:-left-6">
